...
import { ScrollArea } from "@pixa/ui/base-ui/scroll-area";
import { useControllableState } from "@pixa/ui/hooks/use-controllable-state";
...
export function FileUploader(props: FileUploaderProps) {
  ...
  const prevFilesRef = React.useRef<File[] | undefined>(undefined);
  React.useEffect(() => {
    const prevFiles = prevFilesRef.current;
    if (prevFiles) {
      const currentSet = new Set(files);
      prevFiles.forEach((file) => {
        if (!currentSet.has(file) && isFileWithPreview(file)) {
  ...
  React.useEffect(() => {
    return () => {
      const currentFiles = prevFilesRef.current;
