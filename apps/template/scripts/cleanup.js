...
class FeatureCleanup {
  ...
  async cleanup() {
    ...
    const pkgNow = fs.existsSync(pkgPath) ? JSON.parse(fs.readFileSync(pkgPath, "utf8")) : null;
    const clerkGone =
  ...
  cleanNavConfig(navItemsToRemove) {
    ...
    content = content.replace(/,?\s*\{\s*label:\s*['"][^'"]*['"],\s*items:\s*\[\s*\]\s*\}/g, "");
  ...
  applyDocRules(featureName) {
    ...
    for (const rule of rules) {
      ...
      const warn = (anchor) => this.log(`⚠️  Doc anchor not found in ${rule.file}: ${anchor}`);
