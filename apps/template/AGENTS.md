...
13. **Reference apps** - Always inspect **Odoo, Zoho, Petpooja, TMBill** before planning any feature. Compare their flows/models/UI, list gaps vs pixaPOS, and base the `plan.md` on that parity analysis (see root `plan.md`).
14. **Table actions — uniform inventory pattern** - Every list table uses `DropdownMenu modal=false Trigger ghost h-8 w-8 p-0 ellipsis (vertical 3 dots)` with `DropdownContent align=end GroupLabel Actions` → `DropdownItem Update/Edit + Delete`, and `Dialog` for confirm (like `apps/web/features/inventory/components/supplier-list.tsx:124`, `purchase-list.tsx:310`, `raw-material-list.tsx:165`, `menu-list.tsx:153`). Never use inline `Button ghost icon-sm` + `confirm()` directly in `TableCell`.
